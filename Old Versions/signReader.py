...
downMargin = 1;
touchingMargin = 7;
thumbTouchingMargin = 15;

def getSign(fingers, hand, fingerList):
    ...
    if (statusOf['P'] == 'bent'): #PINKY IS BENT. D O C
        if (statusOf['R'] == 'extended'):
            ...
            if (statusOf['I'] == 'extended'):
                ...
                if (isTouching(fingerList, 'T', 'M') or isTouching(fingerList, 'T', 'P')): #if the fingers are touching the thumb
        ...
        if (statusOf['M'] == 'down'):
            ...
            if (statusOf['I'] == 'extended' ):
                if (not isTouching(fingerList, 'M', 'I')):
                    print "You signed a V."
                    return
                else:
                    print "You signed an R or a U. I can't tell yet!"
                    return
...
#
########################################################################
def isTouchingTest(fingers):
    a = 'U'
    dictFingers = dict(fingers)
    for b, f in fingers:
        if (a == 'U'):
            a = b
            continue
        if (b == 'T'or a == 'T'):
            a = b
            continue
        aToB = getDistance(f.bone(2).center, dictFingers[a].bone(2).center)
        print "----\nDistance from %s to %s: %d, width = %d" % (a, b, aToB, f.width)
        if (aToB - f.width < touchingMargin):
            print "%s and %s ARE TOUCHING! " % (a, b)
        a = b
    for b, f in fingers: #loop through again, see which fingers are touching thumb
        if (b == 'T'):
            continue
        aToB = getDistance(dictFingers['T'].tip_position, f.tip_position) #distances from thumb tip to finger tip
        print "----\nDistance from THUMB to %s: %d, width = %d" % (b, aToB, f.width)
        if (aToB - f.width < thumbTouchingMargin):
            print "T and %s ARE TOUCHING! " % (b)

def isTouching(fingers, fingA, fingB):
    
    dictFingers = dict(fingers)
    if (not fingA in dictFingers) or (not fingB in  dictFingers):
        return
    if (fingA != 'T' and fingB != 'T'): #if we're not dealing with the thumb...
        aToB = getDistance(dictFingers[fingA].bone(2).center, dictFingers[fingB].bone(2).center) #get the distance between the center of each finger
        # print "Distance from %s to %s: %d, width = %d" % (fingA, fingB, aToB, dictFingers[fingA].width)
        if (aToB - dictFingers[fingA].width < touchingMargin): #if their distance is not greater than the width of finger A by the constant touchingMargin, they're touching
            print "%s and %s are touching!" % (fingA, fingB)
            return True
    else: #for the thumb, we'll check the tips
        aToB = getDistance(dictFingers[fingA].tip_position, dictFingers[fingB].tip_position) #get the distance between the tips of each finger
        #print "Distance from %s to %s: %d, width = %d" % (fingA, fingB, aToB, dictFingers[fingA].width)
        if (aToB - dictFingers[fingA].width < thumbTouchingMargin): #if theyre close by the constant thumbTouchingMargin, they're touching
            print "%s and %s are touching!" % (fingA, fingB)
            return True
    return False
...
class SampleListener(Leap.Listener):
    ...
    def on_frame(self, controller):
        ...
        if (self.first):
            print("RECORDING... Hit space to get sign, d to print data, 2 to get finger statuses, t to see which fingers are touching, f to print data to file, and p to pause.")
            self.first = False
...
def main():
    ...
    try:
        ...
        while(not(ch == "q")):
            ...
            if (ch == "t" and recording):
                frame1 = frame2;
                frame2 = controller.frame()
                myFingerList = getFingers(frame2)
                isTouchingTest(myFingerList)
            if (ch == " " and  recording):
                frame1 = frame2;
                frame2 = controller.frame()
                myCoords = getCoordList(frame2)
                ...
                #for a, f in fingerStatuses:
                    #  print "%s is %s" % (a,f)
                getSign(fingerStatuses, frame2.hands.frontmost, myFingerList)
